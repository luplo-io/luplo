...
- `summary` is supplied by the caller, never generated by core.
- `sensitivity_hint` is supplied by the caller and limited to `none`,
  `possible`, or `sensitive`.
- `signals` is a JSON object of caller-provided hints such as tags,
  project guesses, mood labels, or confidence values. Core validates
...
| Column | Role |
|---|---|
| `capture_id` | Raw capture used as source material. |
| `target_item_id` | Curated item created from the promotion. |
| `promoted_as` | Requested item type at promotion time. The item row remains authoritative. |
| `created_by`, `created_at` | Promotion attribution and timestamp. |
