...
from luplo.core.errors import QAStateTransitionError
...
@pytest.mark.asyncio
async def test_list_qa_filters_by_status(backend: LocalBackend, fresh_ctx) -> None:
    pid, aid, wu_id = fresh_ctx
...
@pytest.mark.asyncio
async def test_pass_then_cannot_fail(backend: LocalBackend, fresh_ctx) -> None:
    pid, aid, wu_id = fresh_ctx
    q = await backend.create_qa(
        project_id=pid, title="x", actor_id=aid, coverage="human_only", work_unit_id=wu_id
    )
...
async def test_fail_then_retry(backend: LocalBackend, fresh_ctx) -> None:
    pid, aid, wu_id = fresh_ctx
    q = await backend.create_qa(
        project_id=pid, title="r", actor_id=aid, coverage="human_only", work_unit_id=wu_id
    )
...
async def test_assign_qa(backend: LocalBackend, fresh_ctx) -> None:
    ...
    await backend.create_actor(
        id=assignee,
        name="Assignee",
        email=f"{assignee[:8]}@t.com",
    )
    q = await backend.create_qa(
        project_id=pid, title="a", actor_id=aid, coverage="human_only", work_unit_id=wu_id
    )
...
@pytest.mark.asyncio
async def test_list_pending_for_task(backend: LocalBackend, fresh_ctx) -> None:
    pid, aid, wu_id = fresh_ctx
    t = await backend.create_task(project_id=pid, work_unit_id=wu_id, title="T", actor_id=aid)
    q1 = await backend.create_qa(
        project_id=pid,
        title="q1",
        actor_id=aid,
        coverage="human_only",
        target_task_ids=[t.id],
        work_unit_id=wu_id,
    )
    q2 = await backend.create_qa(
        project_id=pid,
...
@pytest.mark.asyncio
async def test_list_pending_for_item(backend: LocalBackend, fresh_ctx) -> None:
    pid, aid, wu_id = fresh_ctx
    target = await backend.create_item(
        ItemCreate(project_id=pid, actor_id=aid, item_type="knowledge", title="target")
    )
    q = await backend.create_qa(
        project_id=pid,
        title="q",
...
@pytest.mark.asyncio
async def test_supersede_revalidates_passed_qa(backend: LocalBackend, fresh_ctx) -> None:
    pid, aid, wu_id = fresh_ctx
    target = await backend.create_item(
        ItemCreate(
            project_id=pid, actor_id=aid, item_type="knowledge", title="design v1", body="..."
        )
    )
    q = await backend.create_qa(
        project_id=pid,
...
async def test_supersede_does_not_revalidate_unrelated_qa(
    ...
    pid, aid, wu_id = fresh_ctx
    target = await backend.create_item(
        ItemCreate(project_id=pid, actor_id=aid, item_type="knowledge", title="target", body="...")
    )
    other = await backend.create_item(
        ItemCreate(project_id=pid, actor_id=aid, item_type="knowledge", title="other", body="...")
    )
    q = await backend.create_qa(
        project_id=pid,
        title="check",
